...
class Cubic(EoSBase):
    ...
    def common(b, pobj):
        ...
        def func_fw(m, j):
            ...
            if ctype == CubicType.PR:
                ...
                return 0.0 #there is no fw for van der Waals EoS
            else:
                raise BurntToast(
    ...
    def enth_mol_phase(blk, p):
        ...
        if pobj._cubic_type == CubicType.VDW:
            #### Check this for vdW - 4/27/21 @12:10pm: BB checked and thinks this equation should work for vdW and the correct equation is being called
#             print('Checking the vdW equation is being used, it is:',((Cubic.gas_constant(blk)*blk.temperature*(Z-1)) + sum(blk.mole_frac_phase_comp[p, j] *
...
#                                      for j in blk.components_in_phase(p)))) 
            return ((Cubic.gas_constant(blk)*blk.temperature*(Z-1)) + sum(blk.mole_frac_phase_comp[p, j] *
                                     get_method(blk, "enth_mol_ig_comp", j)(
                                                blk, cobj(blk, j), blk.temperature)
                                     for j in blk.components_in_phase(p)))
        #TEST - does the additional enthalpy component need to be added here? BB, 4/27/21 @12:28 pm: this changed the temperature, but did not make it match the PR results
...
#         bm = sum(x[xidx, i]*b[i] for i in blk.params.component_list)
        ...
        if ctype == CubicType.VDW:
            ...
            delta = (2*sqrt(a(j)) * sum(x[xidx, i]*sqrt(a(i))*(1-kappa[j, i])
                                           for i in blk.params.component_list))
        else: 
...
#         print('delta=',value(delta))
        return (b/bm*B/(Z-B) - safe_log(Z-B, eps=1e-6) - delta/Z) 
