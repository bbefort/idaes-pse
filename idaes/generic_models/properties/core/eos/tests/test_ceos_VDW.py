...
def m():
    ...
    m.params = DummyParameterBlock(default={
                ...
                "parameter_data": {"VDW_kappa": {("a", "a"): 0.000,
                                                ("a", "b"): 0.000,
                                                ("a", "c"): 0.000,
                                                ("b", "a"): 0.000,
                                                ("b", "b"): 0.000,
...
Al = 0.06810618598835799
Bl = 0.04221875
Av = 0.1208734161522975
Bv = 0.04221875
Zl = 0.9837052366621675
Zv = 0.9337778843717288

Al_eq = 0.612955673895222
Bl_eq = 0.12665625
Av_eq =  1.0878607453706777
Bv_eq =  0.12665625
...
def test_common(m):
    ...
    assert isinstance(m.props[1].VDW_fw, Expression)
    assert len(m.props[1].VDW_fw) == len(m.params.component_list)
    for i in m.params.component_list:
        omega = m.params.get_component(i).omega
        assert value(m.props[1].VDW_fw[i]) == 0.0

    assert isinstance(m.props[1].VDW_a, Expression)
    assert len(m.props[1].VDW_a) == len(m.params.component_list)
    for i in m.params.component_list:
        Tc = m.params.get_component(i).temperature_crit
        Pc = m.params.get_component(i).pressure_crit
        assert pytest.approx(value(m.props[1].VDW_a[i]),
                             rel=1e-5) == value(
            27/64*((const.gas_constant*Tc)**2/Pc) *
            (((1+m.props[1].VDW_fw[i] *
               (1-sqrt(m.props[1].temperature/Tc)))**2)))

    assert isinstance(m.props[1].VDW_b, Expression)
    assert len(m.props[1].VDW_b) == len(m.params.component_list)
    for i in m.params.component_list:
        Tc = m.params.get_component(i).temperature_crit
        Pc = m.params.get_component(i).pressure_crit
        assert value(m.props[1].VDW_b[i]) == value(
            1/8*const.gas_constant*Tc/Pc)

    assert isinstance(m.props[1].VDW_am, Expression)
    assert len(m.props[1].VDW_am) == len(m.params.phase_list)
    for p in m.params.phase_list:
        assert pytest.approx(value(m.props[1].VDW_am[p]),
                             rel=1e-5) == value(
            sum(sum(m.props[1].mole_frac_phase_comp[p, i] *
                    m.props[1].mole_frac_phase_comp[p, j] *
                    sqrt(m.props[1].VDW_a[i]*m.props[1].VDW_a[j]) *
                    (1-m.params.VDW_kappa[i, j])
                    for j in m.params.component_list)
                for i in m.params.component_list))

    assert isinstance(m.props[1].VDW_bm, Expression)
    assert len(m.props[1].VDW_bm) == len(m.params.phase_list)
    for p in m.params.phase_list:
        assert pytest.approx(value(m.props[1].VDW_bm[p]),
                             rel=1e-5) == value(
    ...
    assert isinstance(m.props[1].VDW_A, Expression)
    assert len(m.props[1].VDW_A) == len(m.params.phase_list)
    for p in m.params.phase_list:
        assert pytest.approx(value(m.props[1].VDW_A[p]),
                             rel=1e-5) == value(
                m.props[1].VDW_am[p]*m.props[1].pressure /
                (const.gas_constant*m.props[1].temperature)**2)
    assert pytest.approx(value(m.props[1].VDW_A["Liq"]), rel=1e-5) == Al
    assert pytest.approx(value(m.props[1].VDW_A["Vap"]), rel=1e-5) == Av

    assert isinstance(m.props[1].VDW_B, Expression)
    assert len(m.props[1].VDW_B) == len(m.params.phase_list)
    for p in m.params.phase_list:
        assert pytest.approx(value(m.props[1].VDW_B[p]),
                             rel=1e-5) == value(
                m.props[1].VDW_bm[p]*m.props[1].pressure /
                (const.gas_constant*m.props[1].temperature))
    assert pytest.approx(value(m.props[1].VDW_B["Liq"]), rel=1e-5) == Bl
    assert pytest.approx(value(m.props[1].VDW_B["Vap"]), rel=1e-5) == Bv

    assert isinstance(m.props[1].VDW_delta, Expression)
    assert len(m.props[1].VDW_delta) == 6
    for p in m.params.phase_list:
        for i in m.params.component_list:
            assert pytest.approx(value(m.props[1].VDW_delta[p, i]),
                                 rel=1e-5) == value(
                2*sqrt(m.props[1].VDW_a[i])/m.props[1].VDW_am[p] *
                sum(m.props[1].mole_frac_phase_comp[p, j] *
                    sqrt(m.props[1].VDW_a[j]) *
                    (1-m.params.VDW_kappa[i, j])
                    for j in m.params.component_list))

    assert isinstance(m.props[1].VDW_dadT, Expression)
    assert len(m.props[1].VDW_dadT) == len(m.params.phase_list)
    for p in m.params.phase_list:
        assert pytest.approx(value(m.props[1].VDW_dadT[p]),
                             rel=1e-5) == value(
    ...
    assert isinstance(m.props[1]._VDW_a_eq, Expression)
    assert len(m.props[1]._VDW_a_eq) == len(m.params.component_list)
    for i in m.props[1]._VDW_a_eq:
        Tc = m.params.get_component(i[2]).temperature_crit
        Pc = m.params.get_component(i[2]).pressure_crit
        assert pytest.approx(value(m.props[1]._VDW_a_eq[i]),
                             rel=1e-5) == value(
            27/64*((const.gas_constant*Tc)**2/Pc) *
            (((1+m.props[1].VDW_fw[i[2]] *
               (1-sqrt(m.props[1]._teq[i[0], i[1]]/Tc)))**2)))

    assert isinstance(m.props[1]._VDW_am_eq, Expression)
    assert len(m.props[1]._VDW_am_eq) == len(m.params.phase_list)
    for idx in m.props[1]._VDW_am_eq:
        assert pytest.approx(value(m.props[1]._VDW_am_eq[idx]),
                             rel=1e-5) == value(
            sum(sum(m.props[1].mole_frac_phase_comp[idx[2], i] *
                    m.props[1].mole_frac_phase_comp[idx[2], j] *
                    sqrt(m.props[1]._VDW_a_eq[idx[0], idx[1], i] *
                         m.props[1]._VDW_a_eq[idx[0], idx[1], j]) *
                    (1-m.params.VDW_kappa[i, j])
                    for j in m.params.component_list)
                for i in m.params.component_list))

    assert isinstance(m.props[1]._VDW_A_eq, Expression)
    assert len(m.props[1]._VDW_A_eq) == len(m.params.phase_list)
    for i in m.props[1]._VDW_A_eq:
        assert pytest.approx(value(m.props[1]._VDW_A_eq[i]),
                             rel=1e-5) == value(
                    m.props[1]._VDW_am_eq[i] *
                    m.props[1].pressure /
                    (const.gas_constant *
                     m.props[1]._teq[i[0], i[1]])**2)
    assert pytest.approx(value(m.props[1]._VDW_A_eq[("Vap", "Liq"), "Liq"]),
                         rel=1e-5) == Al_eq
    assert pytest.approx(value(m.props[1]._VDW_A_eq[("Vap", "Liq"), "Vap"]),
                         rel=1e-5) == Av_eq

    assert isinstance(m.props[1]._VDW_B_eq, Expression)
    assert len(m.props[1]._VDW_B_eq) == len(m.params.phase_list)
    for i in m.props[1]._VDW_B_eq:
        assert pytest.approx(value(m.props[1]._VDW_B_eq[i]),
                             rel=1e-5) == value(
                    m.props[1].VDW_bm[i[2]] *
                    m.props[1].pressure /
                    (const.gas_constant *
                     m.props[1]._teq[i[0], i[1]]))
    assert pytest.approx(value(m.props[1]._VDW_B_eq[("Vap", "Liq"), "Liq"]),
                         rel=1e-5) == Bl_eq
    assert pytest.approx(value(m.props[1]._VDW_B_eq[("Vap", "Liq"), "Vap"]),
                         rel=1e-5) == Bv_eq

    assert isinstance(m.props[1]._VDW_delta_eq, Expression)
    assert len(m.props[1]._VDW_delta_eq) == 6
    for idx in m.props[1]._VDW_delta_eq:
        assert pytest.approx(value(m.props[1]._VDW_delta_eq[idx]),
                             rel=1e-5) == value(
            2*sqrt(m.props[1]._VDW_a_eq[idx[0], idx[1], idx[3]]) /
            m.props[1]._VDW_am_eq[idx[0], idx[1], idx[2]] *
            sum(m.props[1].mole_frac_phase_comp[idx[2], j] *
                sqrt(m.props[1]._VDW_a_eq[idx[0], idx[1], j]) *
                (1-m.params.VDW_kappa[idx[3], j])
                for j in m.params.component_list))

    # Check for external function components
    assert isinstance(m.props[1]._VDW_ext_func_param, Param)
    assert m.props[1]._VDW_ext_func_param.value == 2   #Cubic type enumerator
    assert isinstance(m.props[1]._VDW_proc_Z_liq, ExternalFunction)
    assert isinstance(m.props[1]._VDW_proc_Z_vap, ExternalFunction)
...
def test_compress_fact_phase_Liq(m):
    ...
    assert pytest.approx(value(f_Zl(2, Al, Bl)), rel=1e-5) == value( 
...
def test_compress_fact_phase_Vap(m):
    assert isinstance(Cubic.compress_fact_phase(m.props[1], "Vap"),
                      ExternalFunctionExpression)
    assert pytest.approx(value(f_Zv(2, Av, Bv)), rel=1e-5) == value(
...
def test_fug_coeff_phase_comp_Liq(m):
    x = {'a':0.6,'b':0.3,'c':0.1}
    for j in m.params.component_list:
        assert pytest.approx(x[j], rel=1e-5) == value(m.props[1].mole_frac_phase_comp['Liq', j])
            #'a': 0.6
            #'b': 0.3
            #'c': 0.1
    assert pytest.approx(0.9895583812406211, rel=1e-5) == value(
        Cubic.fug_coeff_phase_comp(m.props[1], "Liq", "a"))
    assert pytest.approx(0.9432783429679094, rel=1e-5) == value(
        Cubic.fug_coeff_phase_comp(m.props[1], "Liq", "b"))
    assert pytest.approx(0.9092394415792249, rel=1e-5) == value(
...
def test_fug_coeff_phase_comp_Vap(m):
    y = {'a':0.1,'b':0.2,'c':0.7}
    for j in m.params.component_list:
        assert pytest.approx(y[j], rel=1e-5) == value(m.props[1].mole_frac_phase_comp['Vap', j])
            #'a': 0.1
            #'b': 0.2
            #'c': 0.7
    assert pytest.approx(0.9998514770347608, rel=1e-5) == value(
            Cubic.fug_coeff_phase_comp(m.props[1], "Vap", "a"))
    #y*fug = .099985
    assert pytest.approx(0.9348496639512079, rel=1e-5) == value(
        Cubic.fug_coeff_phase_comp(m.props[1], "Vap", "b"))
    #y*fug =0.0187
    assert pytest.approx(0.8878521483531232, rel=1e-5) == value(
